...
2) 304 Not Modified : 캐시를 목적으로 사용, 
...
```
※ 주의 : 클라이언트가 이미 잘못된 요청, 데이터를 보내고 있기 때문에 똑같은 재시도가 실패
1) 400 Bad Request : 클라이언트가 잘못된 요청을 해서 서버가 요청을 처리할 수 없음
  ex) 요청 구문, 메시지 오류 등등
2) 401 Unauthorized : 클라이언트가 해당 리소스에 대한 인증이 필요함 (인증이 되지 않음)
  401 오류와 함께 응답에 WWW-Authenticate 헤더와 함께 인증 방법을 알려줘야 함
3) 403 Forbidden : 서버가 요청을 이해했지만 승인을 거부함(인증 자격은 있지만 접근 권한이 불충분)
  카카오 2차 코딩테스트에서 발생했던 오류 당시 테스트케이스 파라미터 2를 넘겨줘야 하는데 1을 넘겨줘 발생했음.
4) 404 Not Found : 요청 리소스를 찾을 수 없음
```
> 5xx : 서버 오류, 서버가 정상 요청을 처리하지 못함<br/>
```
 1) 500 Internal Server Error : 서버 문제로 오류 발생 
 2) 503 Server Unavailable : 서비스 이용 불가
    서버가 일시적으로 과부하 또는 예정된 작업으로 잠시 요청을 처리할 수 없을 때
    Retry-After 헤더 필드로 언제 복구되는지 알려줄 수 있음
```
